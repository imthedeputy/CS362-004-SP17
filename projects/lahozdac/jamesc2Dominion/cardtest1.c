...
void testSmithy(struct gameState* state) {
	...
	printf("\n");
	flag = cardEffect(smithy,0,0,0,state,0,0);
	...
	*state = snapshot;
	...
	//Note that handCount should be +2 since Smithy is put into played cards
	if(state->handCount[who] < (snapshot.handCount[who] + 2)) {
		printf("playSmithy() test: FAIL, not enough cards in hand.\n");
		flag = -1;
	}
	...
	if(state->discardCount[who] > 0) {
		printf("playSmithy() test: FAIL to have correct discardCount.\n");
		flag = -1;
	}
	if(flag != -1)
